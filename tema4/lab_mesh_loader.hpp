...
namespace lab{
	...
	struct VertexFormat{
		...
		float next_position_x, next_position_y, next_position_z;
		float next_normal_x, next_normal_y, next_normal_z;
		...
		VertexFormat(float px, float py, float pz, float nx, float ny, float nz, float tx, float ty, float npx, float npy, float npz, 
						float nnx, float nny, float nnz) {
			position_x =px;		position_y =py;		position_z =pz;
			normal_x =nx;		normal_y =ny;		normal_z =nz;
			texcoord_x=tx;		texcoord_y=ty;
			next_position_x = npx; next_position_y = npy; next_position_z = npz;
			next_normal_x = nnx; next_normal_y = nny; next_normal_z = nnz;
		}
	};
	...
	void myLoadObj(std::vector<VertexFormat> vertices1, std::vector<unsigned int> indices1 , std::vector<VertexFormat> vertices2, std::vector<unsigned int> indices2,  unsigned int &vao, unsigned int& vbo, unsigned int &ibo, unsigned int &num_indices) {
		...
		unsigned int gl_vertex_array_object1, gl_vertex_buffer_object1, gl_index_buffer_object1;
		...
		//index buffer object -> un obiect in care tinem indecsii
		glGenBuffers(1,&gl_index_buffer_object1);
		glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, gl_index_buffer_object1);
	...
	void loadObj(const std::string &filename, const std::string &filename1, unsigned int &vao, unsigned int& vbo, unsigned int &ibo, unsigned int &num_indices){

		//incarca vertecsii si indecsii din fisier
		std::vector<VertexFormat> vertices, vertices1;
	...
	void _loadObjFile(const std::string &filename, std::vector<VertexFormat> &vertices, std::vector<unsigned int> &indices){
		...
		while(std::getline(file,line)){
			...
			if(tokens.size()>=4 && tokens[0]=="f"){
				...
				unsigned int index_of_mesh_vertex_of_face = -1;
				...
				for(unsigned int num_token =1; num_token<tokens.size();num_token++){
					...
					//adauga si indecsii
					if(num_token<4){			
						if(num_token == 1) index_of_mesh_vertex_of_face = vertices.size()-1;
						...
						indices.push_back(index_of_mesh_vertex_of_face);
