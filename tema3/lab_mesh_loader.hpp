...
namespace lab{
	...
	struct VertexFormat{
		...
		float color_r, color_g, color_b;						//culoarea unui vertex
		...
		VertexFormat(float px, float py, float pz, glm::vec3 _color){
			position_x = px;		position_y = py;		position_z = pz;
			color_r = _color.x;		color_g = _color.y;		color_b = _color.z;
			normal_x = normal_y = normal_z = 0;
			texcoord_x = texcoord_y = 0;
		}
		VertexFormat(float px, float py, float pz, float nx, float ny, float nz, glm::vec3 _color){
			position_x =px;		position_y =py;		position_z =pz;
			color_r = _color.x;	if(color_r + nx/5 <= 1.0) color_r += nx/5; else color_r = 1.0;	
			color_g = _color.y;	if(color_g + ny/5 <= 1.0) color_g += ny/5; else color_g = 1.0;
			color_b = _color.z; if(color_b + nz/5 <= 1.0) color_b += nz/5; else color_b = 1.0;
			normal_x = nx;		normal_y = ny;		normal_z = nz;
			texcoord_x = texcoord_y = 0;
		}
		VertexFormat(float px, float py, float pz, float tx, float ty, glm::vec3 _color){
			position_x =px;		position_y =py;		position_z =pz;			
			color_r = _color.x;	if(color_r + tx/5 <= 1.0) color_r += tx/5; else color_r = 1.0;	
			color_g = _color.y;	if(color_g + ty/5 <= 1.0) color_g += ty/5; else color_g = 1.0;
			color_b = _color.z; if(color_b + (tx + ty)/5 <= 1.0) color_b += (tx + ty)/5; else color_b = 1.0;
			texcoord_x = tx;	texcoord_y = ty;
			normal_x = normal_y = normal_z = 0;
		}
		VertexFormat(float px, float py, float pz, float nx, float ny, float nz, float tx, float ty, glm::vec3 _color){
			position_x = px;	position_y = py;	position_z = pz;
			color_r = _color.x;	if(color_r + nx/5 <= 1.0) color_r += nx/5; else color_r = 1.0;	
			color_g = _color.y;	if(color_g + ny/5 <= 1.0) color_g += ny/5; else color_g = 1.0;
			color_b = _color.z; if(color_b + nz/5 <= 1.0) color_b += nz/5; else color_b = 1.0;
		...
		VertexFormat operator=(const VertexFormat &rhs){ 
			...
			color_r = rhs.color_r;			color_g = rhs.color_g;			color_b = rhs.color_b;
	...
	//definitie forward
	void _loadObjFile(const std::string &filename, std::vector<VertexFormat> &vertices, std::vector<unsigned int> &indices, glm::vec3 _color);
	...
	void loadObj(const std::string &filename, unsigned int &vao, unsigned int& vbo, unsigned int &ibo, unsigned int &num_indices, glm::vec3 _color){
	
		//incarca vertecsii si indecsii din fisier
		std::vector<VertexFormat> vertices;
		std::vector<unsigned int> indices;
		_loadObjFile(filename, vertices, indices, _color);
	...
	void _loadObjFile(const std::string &filename, std::vector<VertexFormat> &vertices, std::vector<unsigned int> &indices, glm::vec3 color){
		//citim din fisier
			...
			if(tokens.size()>=4 && tokens[0]=="f"){
				...
				for(unsigned int num_token =1; num_token<tokens.size();num_token++){
					...
					if(face_format==1){
						...
						vertices.push_back(VertexFormat(positions[p_index].x, positions[p_index].y, positions[p_index].z, color));
						...
						vertices.push_back(VertexFormat(positions[p_index].x, positions[p_index].y, positions[p_index].z,texcoords[t_index].x, texcoords[t_index].y, color));
						...
						vertices.push_back(VertexFormat(positions[p_index].x, positions[p_index].y, positions[p_index].z,normals[n_index].x, normals[n_index].y, normals[n_index].z, color));
					}else{
						...
						vertices.push_back(VertexFormat(positions[p_index].x, positions[p_index].y, positions[p_index].z,normals[n_index].x, normals[n_index].y, normals[n_index].z, texcoords[t_index].x, texcoords[t_index].y, color));
					}
